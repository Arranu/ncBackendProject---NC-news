...
describe("task 7 - /api/articles/:article_id/comments",()=>{
    describe("POST",()=>{
        ...
        test("error:403 when post request is forbidden (username is not present in users table - violates constraints)",()=>{
            return request(app).post("/api/articles/2/comments").send({userName: "iNeedSleep", body: "text"})
            .expect(403).then(({body})=>{
                expect(body.msg).toBe('Forbidden - possible table constraint violation, check constraints of target table')
            })
        })
    })
})